...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'XapiVerb_tenantId_fkey'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'XapiUri_tenantId_fkey'
  ) THEN
    ALTER TABLE "XapiUri"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'StorylineBridge_tenantId_fkey'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'StorylineBridge_connectionId_fkey'
  ) THEN
    ALTER TABLE "StorylineBridge"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'StorylineVariable_bridgeId_fkey'
  ) THEN
