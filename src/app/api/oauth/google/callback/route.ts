...
export async function GET(req: Request) {
  ...
  if (err) {
    return NextResponse.redirect(
      new URL(`/app/connections?oauth_error=${encodeURIComponent(err)}`, url.origin).toString(),
    );
  }
  if (!code || !state) {
  ...
  if (!tokenRes.ok || !tokenJson?.access_token) {
    return NextResponse.redirect(new URL("/app/connections?oauth_error=token_exchange_failed", url.origin).toString());
  }
